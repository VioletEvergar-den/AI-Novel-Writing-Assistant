...
- 支持 WebDAV 协议同步，可在多台设备之间无缝切换写作。
- 自动同步：可配置定时自动同步，间隔 1-1440 分钟可调。
- 启动时同步：应用启动时自动检查并同步最新数据。
- 手动同步：支持上传、下载和自动选择方向三种模式。
- 冲突处理：基于时间戳自动判断同步方向，减少数据冲突。
...
重大更新：完成前端界面与后端 API 响应消息的全面汉化，降低中文用户的使用门槛。

- 前端设置页、章节运行面板、知识库操作等核心界面文本已全部汉化为中文。
- 后端 22 个路由文件的 API 响应消息已全部汉化，包括世界观、任务中心、知识库、图片生成、书籍分析、Agent 运行、小说管理等模块。
- 所有成功/错误提示消息现在都以中文呈现，中文用户无需再面对英文提示。

重大更新：新增 WebDAV 多设备同步功能，实现跨设备不间断写作。

- 支持 WebDAV 协议，可连接坚果云、Nextcloud、群晖 NAS 等主流 WebDAV 服务。
- 自动同步：可配置定时自动同步，间隔 1-1440 分钟可调。
- 启动时同步：应用启动时自动检查并同步最新数据。
- 手动同步：支持上传、下载和自动选择方向三种模式。
- 基于时间戳的智能冲突处理，自动选择最新版本。

新增：支持打包为 Windows 可执行文件，无需安装 Node.js 环境即可运行。

- 使用 `pnpm build:release` 可一键构建前端、后端并打包为 exe。
- 打包后的程序包含前端静态资源，双击即可启动完整应用。
- 数据库文件默认存储在程序同目录下的 `data` 文件夹。
